...
export default async function handler(request, response) {
    ...
    try {
      ...
      if (!refRes.ok)
        throw new Error(`Failed to fetch latest ref: ${refRes.statusText}`);
      ...
      const shortUrl = `${request.headers.host}/${commitHash}`;
  
      return response.status(200).json({ shortUrl, commitHash });
    } catch (err) {
